...
export function ChatProvider({ children, initialUrlUserId = null }) {
    ...
    try {
      ...
      if (response && response.status === 'Success') {
        ...
        const usersWithFlags = sortedUsers.map(user => ({
          ...
          lastMessageTime: user.last_activity ? new Date(user.last_activity).getTime() : 0,
  ...
  const fetchOnlineUsers = useCallback(async () => {
    ...
    try {
      ...
      if (response && response.status === 'Success') {
        ...
        console.error('Failed to fetch online users:', response?.error_msg || 'Unknown error');
        setOnlineUsers([]);
      }
    } catch (error) {
  ...
  // Fetch chat history
  const fetchChatHistory = useCallback(async (pageNum = 0) => {
    ...
    try {
      ...
      if (response && response.code === 200 && response.data) {
        ...
        const processedMessages = messages.map((msg, index) => ({
          ...
          id: msg.id ? String(msg.id) : `msg_${Date.now()}_${pageNum}_${index}_${Math.random().toString(36).substr(2, 9)}`,
  ...
  const selectUser = useCallback((user) => {
    ...
    setChatUsers(prev => {
      ...
      let updatedUsers = prev.map(u => 
        u.user_id === user.user_id ? updatedUser : u.user_id !== user.user_id ? {
          ...u,
  ...
  }, [isConnected, isLoggedIn, showSuccess, showWarning]);
  ...
  useEffect(() => {
    ...
    const userFromURL = chatUsers.find(user => String(user.user_id) === initialUrlUserId);
